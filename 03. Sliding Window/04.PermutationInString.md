...
class Solution {
    public boolean checkInclusion(String s1, String s2) {
        ...
        char[] verify = s1.toCharArray();
        Arrays.sort(verify);
        int i=0; // j will be i+(winSize-1) always
        while ((i+(winSize-1))<s2.length()) {
            char[] temp = s2.substring(i,i+(winSize-1)+1).toCharArray();
            Arrays.sort(temp);
            if (Arrays.equals(verify,temp)) {
                return true;
            }
            i++;
        }
